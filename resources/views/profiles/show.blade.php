...
<div class="container">
  ...
  <div class="media-body">
    ...
    <div class="btn-group">
      ...
      @endif
      <a class="btn btn-warning" href='{{route('friends.show',$profil->id)}}'>
        Znajomi
        <div class="badge">
      {{DB::table('friends')->where('one_friends_id',$profil->id)->orwhere('two_friends_id',$profil->id)->where('status','success')->count()}}
        </div>
      </a>
      </div>
    </div>
...
<input type="submit" name="" value="Opublikuj" class='btn btn-primary btn-block'>
        ...
        @endif
        @endif
            @foreach(DB::table('posts')->where('ower_id',$profil->id)->orderby('id','desc')->get() as $post)
            <div class="panel panel-default">
<div class="panel-body">
  @if(Auth::user())
